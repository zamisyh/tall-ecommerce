       ...
       <div class="px-5 mt-10" :class="{ 'lg:ml-80 lg:p-5 md:ml-80 md:p-5': drawer }">
            <div class="flex flex-wrap justify-between">
                <div>
                    ...
                    <h3 class="mt-3 text-xl font-thin">This is page for manajement tag</h3>
                </div>
           ...
           <div class="mt-10 mb-5">
            ...
            <div class="flex flex-wrap justify-between">
               ...
               <div>
                    ...
                    @else
                        <button wire:click='$set("openForm", true)' class="btn btn-primary">Create Tag <i class="fa fa-tags" aria-hidden="true"></i></button>
                    @endif
               </div>
            </div>
                    ...
                    <div class="mt-3 overflow-x-auto">
                        @if ($openForm)
                            <div class="p-7">
                                <div class="form-control">
                                    <label class="label">
                                        <span class="label-text">Nama Tag <span class="text-error">*</span></span>
                                    </label>
                                    <input wire:model='name' type="text" class="input input-bordered @error('name')
                                        input-error
                                    @enderror" placeholder="Masukkan nama tag" />
                                    @error('name')
                        ...
                        @else
                            <table class="table w-full rounded-lg table-zebra">
                                ...
                                <tbody>
                                    @forelse ($data['data_tag'] as $key => $item)
                                        <tr>
                                            <td>{{ $data['data_tag']->firstItem() + $key }}</td>
                                            <td>{{ $item->name }}</td>
                                            <td>{{ $item->slug }}</td>
                                            <td>
